...
onmessage = (e) => {
  const file = e.data
  console.log("e sent to worker: ", e.data)
  const fileToUpload = new FileToUpload(file.file, file.type, file.id)
  fileToUpload.upload()
}
...
class FileToUpload {
  ...
  async postChunk(chunk) {
    ...
    try {
      ...
      postMessage([this.fileName, this.progress, this.UploadType, message])
    }
  }
